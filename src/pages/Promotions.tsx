...
const emptyForm = { name: '', promoPrice: '', items: [{ productId: '', quantity: '1' }] as FormItem[] };
...
export function Promotions() {
  ...
  const openEdit = (promo: Promotion) => {
    ...
    setForm({
      ...
      items: promo.items.map(i => ({ productId: String(i.productId), quantity: String(i.quantity) })),
    });
    setShowModal(true);
  };
  ...
  const getProductDisplay = (p: ProductOption) =>
    p.category ? `${p.category} de ${p.name}` : p.name;

  const regularTotal = (promo: Promotion) =>
    promo.items.reduce((sum, i) => sum + (i.product?.price ?? 0) * i.quantity, 0);
      ...
      {promotions.length === 0 ? (
        ...
        <div className="grid gap-4">
          {promotions.map(promo => {
            ...
            return (
              <div key={promo.id}
                className={`group bg-white dark:bg-slate-900 rounded-2xl border transition-all ${promo.isActive
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {promo.items.map((item, i) => (
                          <span key={i} className="text-xs bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-lg">
                            {item.quantity} × {item.product ? getProductDisplay(item.product as ProductOption) : `Producto #${item.productId}`}
                          </span>
                        ))}
                      </div>
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg ring-1 ring-black/5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-8 pt-8 pb-4 sticky top-0 bg-white dark:bg-slate-900 z-10 rounded-t-3xl border-b border-slate-100 dark:border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-400">Definí los productos, cantidades y precio especial</p>
              </div>
            ...
            <div className="px-8 py-6 space-y-5">
              ...
              <div className="space-y-2">
                ...
                {form.promoPrice && form.items.some(i => i.productId) && (() => {
                  const regularSum = form.items.reduce((sum, i) => {
                    if (!i.productId) return sum;
                    const p = products.find(p => p.id === Number(i.productId));
                    return sum + (p?.price ?? 0) * Number(i.quantity || 1);
                  }, 0);
                  const save = regularSum - Number(form.promoPrice);
                  if (save > 0) return (
                    <p className="text-xs text-emerald-600 px-1 mt-1">
                      Precio regular: ${regularSum.toLocaleString('es-AR')} · Ahorro: ${save.toLocaleString('es-AR')}
                    </p>
                  );
                  return null;
                })()}
                ...
                {form.items.map((item, index) => (
                  <div key={index} className="flex gap-2 items-center">
                    <div className="relative flex-1">
                      <select
                        className="w-full appearance-none bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500/30 text-slate-900 dark:text-slate-100 outline-none"
                        value={item.productId}
                        onChange={e => updateFormItem(index, 'productId', e.target.value)}
                      >
                        <option value="">Seleccionar producto...</option>
                        {products.map(p => (
                          <option key={p.id} value={p.id}>{getProductDisplay(p)}</option>
                        ))}
                      </select>
                    ...
                    <div className="relative w-20">
                      <input
                        type="number"
                        min="1"
                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-3 text-sm focus:ring-2 focus:ring-blue-500/30 text-slate-900 dark:text-slate-100 text-center outline-none"
                        value={item.quantity}
                    ...
                    {form.items.length > 1 && (
                      <button
                        ...
                        className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors flex-shrink-0"
                      >
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-sm p-8 ring-1 ring-black/5 space-y-6">
            <div className="text-center">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Las ventas anteriores registradas con esta promo conservan el historial. Solo se elimina la promo hacia adelante.
              </p>
            </div>
